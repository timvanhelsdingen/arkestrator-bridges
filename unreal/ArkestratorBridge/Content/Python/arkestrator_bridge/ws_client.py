...
that is drained on the main thread via Slate tick callbacks.
"""
...
class WebSocketClient:
    ...
    Usage:
        ...
        # In a tick callback: client.poll()
    """
    ...
    def connect(self, url: str, api_key: str = "",
        ...
        # Guard: if already connected to the same server, do nothing.
        if self._is_connected and self._url == url:
            return
        self._url = url
    ...
    def send_bridge_command_result(self, sender_id: str, correlation_id: str,
                                   ...
                                   skipped: int, errors: list[str]) -> None:
    ...
    def poll(self) -> None:
        """Drain incoming queue -- call from main thread (Slate tick)."""
        for _ in range(100):
            try:
    ...
    def _build_url(self, url: str, api_key: str, worker_name: str,
        ...
        # Send OS username so the server knows who is logged in on this machine
        try:
            import getpass
    ...
    def _connection_attempt_urls(self) -> list[tuple[str, str]]:
        """Return primary connect URL plus remote fallback when using a local relay."""
        attempts = [("primary", self._full_url)]
        shared = self._read_shared_config()
    ...
    def _refresh_connect_credentials(self) -> bool:
        ...
        if shared_key and shared_key != self._api_key:
            self._api_key = shared_key
            changed = True

        shared_machine = str(shared.get("machineId", "")).strip()
        if shared_machine and shared_machine != self._machine_id:
            self._machine_id = shared_machine
            changed = True

        shared_worker = str(shared.get("workerName", "")).strip()
        if shared_worker and shared_worker != self._worker_name:
            self._worker_name = shared_worker
            changed = True
    ...
    def _do_connect(self) -> None:
        ...
        for label, connect_url in self._connection_attempt_urls():
    ...
    def _read_loop(self) -> None:
        ...
        while not self._stop_event.is_set() and self._is_connected:
            try:
                ...
                print(f"[ArkestratorBridge] WS disconnect: connection error ({e})")
                break
            except OSError as e:
                print(f"[ArkestratorBridge] WS disconnect: OS error ({e})")
                break
            ...
            if opcode == OPCODE_TEXT:
                ...
                # Parse close code (RFC 6455: first 2 bytes are uint16 big-endian)
                close_code = 0
                close_msg = ""
                if len(payload) >= 2:
                    close_code = struct.unpack("!H", payload[:2])[0]
                    close_msg = payload[2:].decode("utf-8", errors="replace") if len(payload) > 2 else ""
                    # 4001 = replaced by a newer connection from this same bridge.
                    # Do NOT reconnect -- the new connection is already active.
                    if close_code == 4001:
                        self._should_reconnect = False
                        print(f"[ArkestratorBridge] WS disconnect: replaced by newer connection (4001) -- not reconnecting")
                    else:
                        print(f"[ArkestratorBridge] WS disconnect: server closed (code={close_code}{', ' + close_msg if close_msg else ''})")
                else:
                    print("[ArkestratorBridge] WS disconnect: server closed (no code)")
                # Send close back
                try:
