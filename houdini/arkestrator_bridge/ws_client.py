...
class WebSocketClient:
    ...
    def connect(self, url: str, api_key: str = "",
        ...
        # Guard: if already connected to the same server/auth endpoint, do not
        # restart socket threads for metadata-only changes (project/file name).
        # Bridge metadata is now refreshed through editor-context pushes.
        ...
        current_full_url = getattr(self, "_full_url", "")

        # No-op if already connected with identical metadata.
        if self._is_connected and current_full_url == full_url:
            return

        # GUI startup may call register() several times before the first
        # socket finishes connecting. Keep the in-flight connection attempt
        # instead of restarting it on every duplicate startup hook.
        if self.connecting and current_full_url == full_url:
            return
    ...
    def poll(self) -> None:
        """Drain incoming queue -- call from main thread (bpy.app.timers)."""
        for _ in range(100):
            try:
    ...
    def _open_socket(self, host: str, port: int) -> socket.socket:
        """Connect with deterministic address ordering.

        Prefer IPv4 before IPv6 for `.local` hosts because macOS commonly
        resolves IPv6 first even when the server is only reachable on IPv4.
        """
        try:
        ...
        errors: list[str] = []
        for family, socktype, proto, _, sockaddr in ordered_infos:
            raw_sock = None
            try:
                ...
                host_label = sockaddr[0] if isinstance(sockaddr, tuple) and sockaddr else host
        ...
        raise OSError(f"Failed to connect to {host}:{port}: {joined_errors}")
